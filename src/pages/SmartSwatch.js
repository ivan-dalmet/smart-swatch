...
const getColorStringFromHash = () => {
  ...
  return chroma.valid(sanitizedHash)
    ? chroma(sanitizedHash).name()
    : DEFAULT_USER_COLOR_STRING;
};
...
const getUserColorChroma = (
  ...
  chroma.valid(colorString)
    ? chroma(colorString)
    : chroma(fallbackValue);

export const SmartSwatch = () => {
  const [userColorInput, setUserColorInput] = useState(
    getColorStringFromHash(),
  );
  const [userColorChroma, setUserColorChroma] = useState(
    getUserColorChroma(userColorInput.trim()),
  );
  const { colorMode, toggleColorMode } = useColorMode();

  const { onCopy, hasCopied } = useClipboard(window.location);

  const updateColorHashDebounced = React.useCallback(
    debounce((newUserColorInput) => {
      ...
      if (
        chroma.valid(newUserColorInput) &&
        newUserColorInput !== getColorStringFromHash()
      ) {
        const newHash = newUserColorInput.startsWith("#")
          ? newUserColorInput
          : `#${newUserColorInput}`;
  ...
  React.useEffect(
    function updateColorChromaFromInputEffect() {
  ...
  return (
    <>
            ...
            <Flex>
              <InputGroup flex={1}>
                ...
                <Input
                  id="color"
                  onChange={(e) =>
                    handleUserColorChange(e.target.value)
                  }
            ...
            <PopoverContent zIndex={4} w="auto" color="black">
              <SketchPicker
                ...
                onChangeComplete={(color) =>
                  handleUserColorChange(color.hex)
                }
                disableAlpha
      ...
      >
        ...
        <Text>-</Text>
        <Link
          href="https://github.com/ivan-dalmet/smart-swatch"
          color={colors[5].hex()}
        >
          GitHub
        </Link>
